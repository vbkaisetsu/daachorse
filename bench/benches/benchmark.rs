...
const BUILD_MEASURE_TIME: Duration = Duration::from_secs(50);
...
fn criterion_ipadic_build(c: &mut Criterion) {
    let mut group = c.benchmark_group("ipadic/build");
    group.sample_size(BUILD_SAMPLE_SIZE);
    group.warm_up_time(BUILD_WARM_UP_TIME);
    group.measurement_time(BUILD_MEASURE_TIME);
    group.sampling_mode(SamplingMode::Flat);
    let mut patterns = load_file("data/ipadic/ipadic");
    patterns.sort_unstable();
...
fn criterion_ipadic_exact_match(c: &mut Criterion) {
    let mut group = c.benchmark_group("ipadic/exact_match");
    group.sample_size(SEARCH_SAMPLE_SIZE);
    group.warm_up_time(SEARCH_WARM_UP_TIME);
    group.measurement_time(SEARCH_MEASURE_TIME);
    group.sampling_mode(SamplingMode::Flat);
...
fn criterion_ipadic_find(c: &mut Criterion) {
    let mut group = c.benchmark_group("ipadic/find");
    group.sample_size(SEARCH_SAMPLE_SIZE);
    group.warm_up_time(SEARCH_WARM_UP_TIME);
    group.measurement_time(SEARCH_MEASURE_TIME);
    group.sampling_mode(SamplingMode::Flat);
...
fn criterion_ipadic_find_overlapping(c: &mut Criterion) {
    let mut group = c.benchmark_group("ipadic/find_overlapping");
    group.sample_size(SEARCH_SAMPLE_SIZE);
    group.warm_up_time(SEARCH_WARM_UP_TIME);
    group.measurement_time(SEARCH_MEASURE_TIME);
    group.sampling_mode(SamplingMode::Flat);
...
criterion_group!(
    ...
    criterion_ipadic_build,
    criterion_ipadic_exact_match,
    criterion_ipadic_find,
    criterion_ipadic_find_overlapping,
);
